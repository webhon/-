...
// ----------------------------------------------------------------------------
// 부모글 - 자식글의 정렬해서 조회하기 위한 쿼리 수정 시작
// ----------------------------------------------------------------------------
//bbs테이블에서 목록을 가져옵니다. (위의 쿼리문 사용예와 비슷합니다 .) 
//parentno를 내림차순으로 정렬한 후 다시 replylv를 기준으로 오름차순 정렬합니다.
//그러면 정렬 결과가 바로 화면에 보여줄 수 있는 상태로 조회가 됩니다.
$query="select * from $tablename order by parentno desc, replylv limit $offset, $list_num";
// ----------------------------------------------------------------------------
// 부모글 - 자식글의 정렬해서 조회하기 위한 쿼리 수정 종료
// ----------------------------------------------------------------------------
...
while ($array=mysql_fetch_array($result)) { 
	...
	if($indent!="") $indent = $indent . "<b>┗</b>";
	echo " 
	<tr> 
		<td width=30>$cur_num</td> 
		<td width=490>$indent<a href='bbs_view.php?number=$array[number]'>$array[subject]</a></td> 
...
?>
<form name="x">
<tr><td colspan="5" align="right"><select ><option>글 제목</option><option>글 내용</option></select> <input type="text" size="20" style="border:1px solid black">
			<input type="button" name="btn_write" class="ahnbutton" value="게시물 검색" title="" onMouseOver="goLite(this.form.name,this.name)" onMouseOut="goDim(this.form.name,this.name)" onClick="location.href='bbs_write.php'">
</td></tr>
</form>
</table>
<table border=0 width="680">
